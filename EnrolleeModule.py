...
def display_member_utilization(mem_id):   
    ...
    member_utilization = utilization_data.loc[
            ...
            (utilization_data['MemberNo'] == mem_id) &
            (utilization_data['New Approval Status'] == 'APPROVED'),
            ['AvonPaCode','ProviderName', 'EncounterDate','PAIssueDate', 'Benefit', 'Diagnosis', 'Speciality', 'ServiceDescription','State', 'ProviderManager', 'FinalPAAmount', 'ApprovedPAAmount' ]
    ...
    if membername is not None and options == 'Enrollee Bio-Data':  
        ...
        utilization_summary = member_utilization.groupby('Benefit')['FinalPAAmount'].sum().__round__(2)
        ...
        for benefit, keywords in benefit_keywords.items():
            ...
            benefit_utilization = benefit_data['FinalPAAmount'].sum()
            ...
            if limit is not None:
                ...
                with st.expander(f"View Utilization Details for {benefit}"):
                    if not benefit_data.empty:
                        st.dataframe(benefit_data[['PAIssueDate', 'ServiceDescription', 'FinalPAAmount', 'ApprovedPAAmount']])
        ...
        remaining_utilization = remaining_data['FinalPAAmount'].sum()
        ...
        if limits["Total Limit"] is not None:
            ...
            with st.expander('View details for all other Utilization'):
                if not remaining_data.empty:
                    st.dataframe(remaining_data[['PAIssueDate', 'ServiceDescription', 'FinalPAAmount', 'ApprovedPAAmount']])
                else:
